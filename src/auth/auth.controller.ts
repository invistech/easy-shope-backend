...
import { CreateAdminUserAuthDto, LoginAdminUserAuthDto } from './dto/create-admin-auth.dto';
...
export class AuthController {
  ...
  async createAdmin(@Body() createAdminUserAuthDto: CreateAdminUserAuthDto) {
    ...
    const _hashPassword = await bcrypt.hash(createAdminUserAuthDto.password, _hash);
    const _data = await {
  ...
  @Patch('admin/:id')
  updateAdmin(@Param('id') id: string, @Body() updateAdminUserAuthDto: UpdateAdminUserAuthDto) {
  ...
  @Patch('customer/:id')
  updateCustomer(@Param('id') id: string, @Body() updateCustomerAuthDto: CreateCustomerAuthDto) {
